...
const ReportDetailsComponent = ({ report }: { report: Report }) => {
  ...
  const upvote = async () => {
    ...
    try {
      const res = await axios.patch(`/api/report/upvote/${report._id}`, {
        userAddress,
      });
  ...
  const downvote = async () => {
    ...
    try {
      const res = await axios.patch(`/api/report/downvote/${report._id}`, {
        userAddress,
      });
      ...
      console.log(error);
      toast.error("Error downvoting report");
    }
  };
    ...
    <div className="flex gap-4 items-start">
      ...
      <div className="space-y-6 flex flex-col justify-center max-w-4xl">
        ...
        <p className="text-red-600 font-medium text-sm">
          ReportedBy: <span className="text-gray-300">{report.reportedBy}</span>
        </p>
